...
以下は、`/api/v1/agent/chat` がプランを作る際に必ず守るべき運用ルール。

### 1. プロジェクトに論文を入れる前提

- `POST /api/v1/projects/{project_id}/papers` で追加する論文は、先にライブラリに存在していること。
- そのため、基本フローは以下の順番を守ること。
  1. `GET /api/v1/search/papers` で検索
  2. `POST /api/v1/library/{paper_id}/like` でライブラリ保存
  3. `POST /api/v1/projects/{project_id}/papers` でプロジェクト追加

### 2. 新規作成プロジェクトのID扱い

- プロジェクト作成直後は、必ず `POST /api/v1/projects` のレスポンス `id` を後続処理で使うこと。
- タイトル文字列や推測IDを `project_id` に使ってはいけない。

### 3. 複数件（例: 10本）処理の標準手順

- N本追加する場合は、検索件数をN以上で取得し、各論文に対して以下を繰り返すこと。
  1. Like（未保存の場合）
  2. プロジェクト追加
  3. メモ作成（論文ref付き）
- 途中失敗があっても残りは継続し、最後に成功件数/失敗件数を返すこと。

### 4. メモ作成の最小要件

- 論文要約メモは `refs=[{ref_type:\"paper\", ref_id:paper_id}]` を必須とする。
- 本文は最低でも `title`, `authors`, `year`, `abstract要約` を含めること。

### 5. インジェスト起動の注意

- `pdf_url` が不確実な場合、取り込み（ingest）失敗の原因になり得る。
- 自動処理時は、PDF直リンクが確認できる論文のみ取り込みを優先すること。
