...
export function AgentChatWidget() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      const assistantMessage: UiMessage = {
        id: `a-${Date.now()}`,
        role: "assistant",
        content: `${res.reply}\n\n${summarizeSteps(res.steps)}`,
        meta: { plan: res.plan, steps: res.steps },
      };
  ...
  return (
    <>
      ...
      <section
        className={`fixed bottom-24 right-5 z-50 flex h-[70vh] w-[min(95vw,420px)] flex-col overflow-hidden rounded-2xl border border-border bg-background/95 shadow-2xl backdrop-blur transition-all duration-300 ${
          open
